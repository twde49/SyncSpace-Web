    ...
    <div class="w-full px-4 h-full">
        <div class="flex justify-between items-center text-lg mb-8">
            <div class="relative flex justify-center items-center megaFont textColorSecondary">
                <button class="flex pt-4 pl-4 justify-center items-center" @click="toggleMonthDropdown">
            ...
            <div class="ml-2">
                ...
                <div id="tooltip-today" role="tooltip"
            ...
            <div class="textColorSecondary ml-2">
                <Icon name="ph:plus-square" size="1.4em" class="TextColorSecondary cursor-pointer"
                    data-tooltip-target="tooltip-add-event" @click="openAddEventModal" />
                <div id="tooltip-add-event" role="tooltip"
                    class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium textColorWhite transition-opacity duration-300 bgColorPrimary rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
            ...
            <div class="flex">
                <button @click="prevWeek" class="text-2xl">
                    <Icon class="bgColorSecondary" name="ph:arrow-circle-left" size="1.2em" />
                </button>
                <button @click="nextWeek" class="text-2xl">
                    <Icon class="bgColorSecondary" name="ph:arrow-circle-right" size="1.2em" />
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-7 gap-1.5">
            <div v-for="(day, index) in visibleDays" :key="index" :class="[
            ...
            ]" @click="openDay(day)" :data-tooltip-target="'tooltip-day-' + index" @mouseenter="hoveredDay = day as Date"
                ...
                <div :id="'tooltip-day-' + index" role="tooltip"
                    class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium textColorWhite transition-opacity duration-300 bgColorPrimary rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
        ...
        <div v-if="showDayModal" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[1000]"
            @click.self="closeDayModal">
            <div class="bgColorWhite w-[90%] max-w-[800px] max-h-[90vh] rounded-lg overflow-y-auto shadow-lg">
                <div
                    ...
                    <div class="flex items-center">
                        <button @click="openAddEventModalForDay(selectedDay)"
                            class="mr-4 bgColorPrimary textColorWhiteFull px-3 py-1 rounded flex items-center">
                            <Icon name="ph:calendar-plus" class="mr-1" />
                            Ajouter
                        </button>
                        <button @click="closeDayModal" class="bg-transparent border-0 text-2xl cursor-pointer">
                            &times;
                        </button>
                    </div>
                </div>
                <div class="p-4 modalSize">
                    <div class="flex flex-col gap-2.5">
                        <div v-if="selectedDay && getEventsForDay(selectedDay).filter((event: Event) => event.isAllDay).length > 0"
                            class="mb-2.5">
                            <h3 class="font-bold text-lg">
                                Événements du jour entier ({{getEventsForDay(selectedDay).filter((event: Event) =>
                                event.isAllDay).length }})
                            </h3>
                            <details>
                                <summary class="cursor-pointer">Afficher/Masquer les événements du jour entier</summary>
                                <div class="flex flex-col gap-2">
                                    <div v-for="event in getEventsForDay(selectedDay).filter((event: Event) => event.isAllDay)"
                                        :key="event.id"
                        ...
                        <div v-for="hour in 24" :key="hour - 1"
                            ...
                            <div class="w-[60px] text-right pr-2.5 font-bold text-[var(--color-black)]">
                                {{ hour - 1 }}:00
                            </div>
                            <div class="flex-grow pl-2.5 border-l border-[var(--color-primary)]">
                                <div v-for="event in selectedDay ? getEventsForHour(selectedDay, hour - 1).filter((event: Event) => !event.isAllDay) : []"
                                    :key="event.id" class="textColorWhiteFull p-2 rounded mb-1.5 cursor-pointer"
                        ...
                        ">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <div v-for="user in selectedEvent.participants" :key="user.id"
                                    class="flex items-center px-2.5 py-1.5 rounded-full text-sm" :style="{
                                        backgroundColor:
        ...
        <div v-if="showAddEventModal"
            ...
            <div class="bgColorWhite w-[90%] max-w-[800px] max-h-[90vh] rounded-lg overflow-y-auto shadow-lg">
                <div
                    ...
                    <button @click="closeAddEventModal" class="bg-transparent border-0 text-2xl cursor-pointer">
                        &times;
                    </button>
                </div>
                <div class="p-4 modalSize">
                    <form @submit.prevent="saveEvent" class="text-[var(--color-black)]">
                        <div class="mb-4">
                            ...
                            <input type="text" id="event-title" v-model="newEvent.title"
                                class="w-full p-2 rounded bg-[var(--color-black)] textColorWhite" required />
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="event-description" v-model="newEvent.description"
                                class="w-full p-2 rounded bg-[var(--color-black)] textColorWhite min-h-[100px] resize-y"></textarea>
                        </div>

                        <div class="mb-4">
                            <div class="flex items-center">
                                <input type="checkbox" id="isAllDay" v-model="newEvent.isAllDay" class="mr-2" />
                        ...
                        <div class="flex flex-col gap-4 mb-4">
                            <div class="mb-2.5">
                                ...
                                <div class="flex gap-2.5 items-center">
                                    <input type="date" id="start-date" v-model="newEvent.startDateInput"
                                        class="w-full p-2 rounded bg-[var(--color-black)] textColorWhite flex-2"
                                        required />
                                    <input v-if="!newEvent.isAllDay" type="time" id="start-time"
                            ...
                            <div class="mb-2.5">
                                ...
                                <div class="flex gap-2.5 items-center">
                                    <input type="date" id="end-date" v-model="newEvent.endDateInput"
                                        class="w-full p-2 rounded bg-[var(--color-black)] textColorWhite flex-2"
                                        required />
                                    <input v-if="!newEvent.isAllDay" type="time" id="end-time"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="color" id="event-color" v-model="newEvent.color"
        ...
        <div v-if="showEditEventModal"
            ...
            <div class="bgColorWhite w-[90%] max-w-[800px] max-h-[90vh] rounded-lg overflow-y-auto shadow-lg">
                <div
                    ...
                    <button @click="closeEditEventModal" class="bg-transparent border-0 text-2xl cursor-pointer">
                        &times;
                    </button>
                </div>
                <div class="p-4 modalSize">
                    <form @submit.prevent="updateEvent" class="text-[var(--color-black)]">
                        <div class="mb-4">
                            ...
                            <input type="text" id="edit-event-title" v-model="editEventData.title"
                                class="w-full p-2 rounded bg-[var(--color-black)] textColorWhite" required />
                        </div>

                        <div class="mb-4">
                            <label for="edit-event-description" class="block mb-1.5 font-bold">
                                Description
                            </label>
                            <textarea id="edit-event-description" v-model="editEventData.description"
                                class="w-full p-2 rounded bg-[var(--color-black)] textColorWhite min-h-[100px] resize-y"></textarea>
                        </div>

                        <div class="mb-4">
                            <div class="flex items-center">
                                <input type="checkbox" id="edit-isAllDay" v-model="editEventData.isAllDay"
                                    class="mr-2" />
                        ...
                        <div class="flex flex-col gap-4 mb-4">
                            <div class="mb-2.5">
                                ...
                                <div class="flex gap-2.5 items-center">
                                    <input type="date" id="edit-start-date" v-model="editEventData.startDateInput"
                                        class="w-full p-2 rounded bg-[var(--color-black)] textColorWhite flex-2"
                                        required />
                                    <input v-if="!editEventData.isAllDay" type="time" id="edit-start-time"
                            ...
                            <div class="mb-2.5">
                                ...
                                <div class="flex gap-2.5 items-center">
                                    <input type="date" id="edit-end-date" v-model="editEventData.endDateInput"
                                        class="w-full p-2 rounded bg-[var(--color-black)] textColorWhite flex-2"
                                        required />
                                    <input v-if="!editEventData.isAllDay" type="time" id="edit-end-time"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="color" id="edit-event-color" v-model="editEventData.color"
                                class="w-full h-10 p-0 rounded bg-[var(--color-black)] textColorWhite" />
                        </div>
