...
export function useMusicPlayer() {
  ...
  try {
    ...
    console.error('An unexpected error occurred while checking favorite quantity:', err);
  }
	}
	...
	async function loadNextRecommendation() {
		...
		if (isLoadingRecommendation.value) {
			console.log("loadNextRecommendation: Already loading recommendation, returning.");
			return;
		}
		...
		try {
			...
			console.log("loadNextRecommendation: Fetching recommendation from URL:", recommendationUrl);
			const response = await useAuthFetch(
				recommendationUrl,
			);

			const recommendation = response.data.value as Track;
			console.log("loadNextRecommendation: Received recommendation:", recommendation);
			...
			if (exists) {
				offsetDouble.value++;
				console.log(`loadNextRecommendation: Incrementing offsetDouble to ${offsetDouble.value} and retrying.`);
				loadNextRecommendation();
			} else {
				playlist.value.push({
				...
				console.log("loadNextRecommendation: New track added to playlist:", recommendation);
				setCurrentMusicToUser(recommendation);
				console.log("loadNextRecommendation: Set current music to user with new recommendation.");
			...
			console.error("loadNextRecommendation: Error loading recommendations", error);
		} finally {
			isLoadingRecommendation.value = false;
			isLoadingNextTrack.value = false;
			console.log("loadNextRecommendation: isLoadingRecommendation and isLoadingNextTrack set to false (finally block).");
		}
	}
	...
	function initializePlaylist() {
		if (userStore.currentTrack) {
			const track = {
				...
				artist: userStore.currentTrack.artist || "",
				coverUrl:
