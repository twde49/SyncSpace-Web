<template>
    <div v-if="notifications.length > 0" class="flex flex-col gap-2 max-h-52 scrollable">
        <div v-for="notification in notifications" :key="notification.id">
            <div class="bg-white rounded-lg shadow-md p-4 mb-3 relative border border-gray-200">
                <div class="flex justify-between items-start">
                    <h3 class="font-semibold text-lg text-gray-800">{{ notification.title }}</h3>
                    <button @click="$emit('readNotification', notification.id)" class="text-gray-500 hover:text-gray-700">
